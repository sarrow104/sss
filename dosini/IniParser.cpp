...
void trimRange(const std::string& line, int &left, int &right) {
    while (left < right && (std::isspace(line[left]) != 0)) {
        left++;
    }
    while (left < right && (std::isspace(line[right - 1]) != 0)) {
        right--;
    }
}
                     ...
                     int pos)
{
    ...
    if (0 == sscanf(line.c_str() + pos, " %n%*[^=]%n=%n", &key_left, &key_right, &last) && last > 0) {
        ...
        value_left = last + pos;
        value_right = int(line.size()); // 这里不是很准确；这个赋值，是以line仅包含一行文本的前提而定的；如果……
