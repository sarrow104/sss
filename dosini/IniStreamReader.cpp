...
IniStreamReader::IniStreamReader()
    : m_in(nullptr), m_ranges{0}, m_type_state(TYPE_INITIATE), m_type_to_skip(TYPE_INITIATE)
{}

IniStreamReader::~IniStreamReader() = default;
...
void IniStreamReader::readNext()
{
    ...
    if ((this->m_type_state & this->m_type_to_skip) != 0) {
        this->readNext();
    }
}
