...
namespace sss {
    ...
    inline IniStreamReader::elementType operator | (IniStreamReader::elementType l, IniStreamReader::elementType r) {
        return IniStreamReader::elementType(unsigned(l) | unsigned(r));
    }
    // std::ostream 有两种，一种是可以回溯(std::ifstream&)；一种是不可回溯(std::cin&)
    // 对比：
