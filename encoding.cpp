...
namespace sss {
    ...
    // NOTE 所谓 compatible 是说，一种编码如果写成另外一种编码，是否可以省略转换的过程？
    ...
    // 为一个"代理对"；
    //
    // 简单的说，UTF-16可看成是UCS-2的父集。在没有辅助平面字符（surrogate code points）前，UTF-16与
    ...
    bool Encoding::isCompatibleWith(std::string from, std::string to)
    {
        ...
        else if ((from == "cp936" && (to == "gb2312" || to == "gbk" || to == "gb18030")) ||
    ...
    // 的，其实是字符序列；
    //
    // 如果，一个 gb18030 编码的字符串，里面没有简体中文字符，全部都是由繁体字符组成的，那么这个
    ...
    std::string Encoding::dectect(const std::string& content)
    {
        ...
        if (uchardet_handle_data(ud, content.c_str(), content.length()) != 0) /* 如果样本字符不够，那么有可能导致分析失败 */
        {
    ...
    std::string Encoding::encodings(const std::string& content, const std::string& encodings)
    {
        std::string encoding;
