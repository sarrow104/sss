...
namespace sss {
    namespace xml {
        ...
        // 因为可以通过 sss::xml::node * 节点指针来操作、修改xml结构，这就导致
        ...
        sss::xml::xmlcfg_file::iterator xmlcfg_file::sub_begin(const std::string& path)
        {
            sss::xml::node * p_node = my_doc.locate(path);
            if (p_node) {
        ...
        sss::xml::xmlcfg_file::iterator xmlcfg_file::sub_end(const std::string& path)
        {
            sss::xml::node * p_node = my_doc.locate(path);
            if (p_node) {
        ...
        void sss::xml::xmlcfg_file::print(std::ostream& o)
        {
            this->my_doc.print(o);
        }

        void sss::xml::xmlcfg_file::update()
        {
            if (this->is_modified) {
        ...
        sss::xml::xml_doc& sss::xml::xmlcfg_file::get_xmldoc() {
            return this->my_doc;
        }
    }
}
