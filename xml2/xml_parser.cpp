...
namespace sss {
    ...
    namespace xml2 {
        ...
        {
        public:
            AttNameValidator(sss::xml2::xml_parser* xp)
                : _xp(xp)
            {
                assert(_xp);
            ...
            void checkIn()
            {
                ...
                if (_attribute_names.find(attribute_name) != _attribute_names.end()) {
                    throw std::runtime_error("encounter the same attribute_name:" + attribute_name);
                }
                _attribute_names.insert(attribute_name);
            }

        protected:
            sss::xml2::xml_parser * _xp;
        ...
        xml2::node * xml_parser::parse(const std::string& xml_str_data)
        {
        ...
        xml2::node * xml_parser::Seg2XMLsubtree(xml2::xml_doc * pdoc, xml_parser::SegmentXML * seg_root)
        {
            SSS_LOG_FUNC_TRACE(sss::log::log_DEBUG);
            try {
                assert(pdoc);
                ...
                this->p_root = new xml2::xml_doc; // TODO 用handle来包裹，避免内存泄露
                this->p_cur = p_root;

                this->Seg2XML_element(seg_root);  // 将current以element元素，一次解析下去；

                xml2::node_list_t tmp_nodes;
                std::swap(tmp_nodes, this->p_root->node_list);
                delete this->p_root;
                assert(tmp_nodes.size() == 1u);
                xml2::node * ret = tmp_nodes[0];
        ...
        xml2::node * xml_parser::Seg2XML(xml_parser::SegmentXML * seg_root)
        {
            SSS_LOG_FUNC_TRACE(sss::log::log_DEBUG);
            try {
                assert(seg_root);
                ...
                if (!this->p_document) {
                    this->p_document = new xml2::xml_doc;
        ...
        bool xml_parser::Seg2XML_prolog(xml_parser::SegmentXML *& current)
        {
            ...
            for (xml_parser::SegmentXML * seg = current->firstChild(); seg; seg = seg->nextSibling()) {
                ...
                case xml_parser::falldown_name_table::PARSE_DOCTYPEDECL:
                    ...
                    this->p_document->doctype(this->p_doctype);
                    break;
...
out_side:
        ...
        bool xml_parser::Seg2XML_element(xml_parser::SegmentXML *& current)
        {
            ...
            xml2::node * p_cur_bak = this->p_cur;
            ...
            default:
                throw std::runtime_error("unexpect seg node :" +
        ...
        bool xml_parser::Seg2XML_content(xml_parser::SegmentXML *& current)
        {
            ...
            for (xml_parser::SegmentXML *& seg = current; seg;) {
                ...
                case xml_parser::falldown_name_table::PARSE_REFERENCE:
                    throw "xml_parser::falldown_name_table::PARSE_REFERENCE not support.";
                    break;
...
out_side:
        ...
        xml2::node * xml_parser::parse(std::string::const_iterator s_ini,
        ...
        xml2::node * xml_parser::parseSubtree(xml2::xml_doc * pdoc, std::string::const_iterator s_ini, std::string::const_iterator s_fin)
        {
            try {
                this->set_sbeg(s_ini);
                this->set_p_it(s_ini);
        ...
        {
            for (sss::xml2::bom_str_table_t::iterator it = sss::xml2::bom_str_table.begin();
                 it != sss::xml2::bom_str_table.end();
                 ++it)
            {
        ...
        int xml_parser::parse_element(std::string::const_iterator& s_beg, std::string::const_iterator s_end)
        {
                         ...
                         this->parse_ETag(s_beg, s_end)))
            {
                ...
                if (STag_name_seg->get_slice() != ETag_name_seg->get_slice()) {
                    std::ostringstream oss;
                    oss << " 开始标签名`" << STag_name_seg->get_slice() << "`!=`" << "结束标签名`"
                        << ETag_name_seg->get_slice() << "`"
                        << std::endl;
