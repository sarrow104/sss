...
// 解析xml文件；并生成一个xml_doc对象或者xml2::node * 对象；
namespace sss {
namespace xml2 {
...
public:
    using skipper_t = sss::stream::skipper<std::string::const_iterator>;
    using Rewinder_t = sss::stream::Rewinder<std::string::const_iterator>;
...
public:
    ...
    public:
        ...
        void trim()
        {
            while (this->start != this->end &&
                   (std::isspace(*(this->s_beg + this->start)) != 0)) {
                this->start ++;
                this->ws_bytes_before ++;
            }
            while (this->start != this->end &&
                   (std::isspace(*(this->s_beg + this->end - 1)) != 0)) {
                this->end --;
                this->ws_bytes_after ++;
            }
        }
        ...
        {
            return {this->s_beg + this->start, this->s_beg + this->end};
        }

        std::string get_slice(int beg_off, int end_off) const
        {
            return {this->s_beg + this->start + beg_off, this->s_beg + this->end - end_off};
        }
        ...
        {
            return this->Base_t::appendChild(child, nullptr);
        }
    ...
    public:
        ...
        bool isOK() const {
            return (this->seg == nullptr) || this->seg->get_end() != -1;
        }
    ...
    {
    public:
        using value_type = std::map<XMLpathMemKey, SegmentXML *>;
    ...
    public:
        void clear();
        bool add(const XMLpathMemKey&, SegmentXML*) noexcept(false);
        bool has_trival(const XMLpathMemKey&, SegmentXML *&);
        bool usePath(const XMLpathMemKey&);
    };

    // 本质上，是将传入的字符串，解释、并分析，最后组建一个xml2::node 对
    // 象；
    // NOTE 可以是xml_doc类型；
    // 那么，就需要进行分析；
    //
...
public:
    ...
    // 并返回该对象
    xml2::node * parse(std::string::const_iterator s_ini,
                       std::string::const_iterator s_fin);

    xml2::node * parse(const std::string&);
    ...
    xml2::node * parseSubtree(xml_doc * doc, const std::string& str)
    {
        return this->parseSubtree(doc, str.begin(), str.end());
    }

    xml2::node * parseSubtree(xml_doc *,
                              std::string::const_iterator s_ini,
                              std::string::const_iterator s_fin);

protected:
    // 完成转换；
    xml2::node * Seg2XML(SegmentXML * seg_root); // == Seg2XML_document()
    xml2::node * Seg2XMLsubtree(xml2::xml_doc * pdoc, xml_parser::SegmentXML * seg_root);
...
public:
    using parser_Func_t = int (xml_parser::*)(std::string::const_iterator &, std::string::const_iterator);
...
//                        << " msg = " << msg << std::endl;
            this->set_last_err(this->get_offset(*p_it),
                               last_func_name != nullptr ? last_func_name : "",
                               msg);
        }
...
private:
    ...
    // 参考 jsonpp::JHandle
    xml2::node *                p_root;         // 根节点
    xml2::node *                p_cur;          // 临时工作节点
    xml2::node *                p_info;         // info节点
    xml2::node *                p_doctype;      // doctype节点
    xml2::xml_doc *             p_document;     // doctype节点
