...
// TODO
// xmlcfg_file 应该作为sss::xml2::node * 的一个handle来使用；并且，提供了一些适用的sugar 方法：
...
namespace sss {
    namespace xml2 {
        ...
        {
        public:
            explicit xmlkey(sss::xml2::node * p)
                : p_node(p)
            {
            }
            ~xmlkey()
        ...
        public:
            template<typename T>
                T get(const std::string& path)
                {
                    try {
                        T value;
                        sss::xml2::node * p_path = p_node->locate(path);
                        if (p_path) {
        ...
        public:
            ...
            {
            private:
                sss::xml2::node_list_t::iterator index;
                sss::xml2::node * p_node;
            ...
            public:
                ...
                iterator(sss::xml2::node * p, bool is_begin)
                    : p_node(p)
                {
                    if (is_begin) {
        ...
        {
        public:
            typedef sss::xml2::node_list_t::iterator iterator;
        ...
        public:
            ...
            inline std::string set_string(const std::string& path, const std::string& value)
            {
                try {
                    std::string ret;
                    sss::xml2::node * p_node = my_doc->verify_path(path);
                    if (p_node) {
                        ret = p_node->inner_text();
                        sss::scoped_ptr<sss::xml2::node> tmp_node(my_doc->create_text(value));
                        if (tmp_node.get()) {
                            p_node->clear();
                            p_node->append_child(tmp_node.release());
                            this->mark_modified();
                        }
                    }
                    return ret;
            ...
            inline std::string set_cdata(const std::string& path, const std::string& value)
            {
                try {
                    ...
                    if (p_node) {
                        ret = p_node->inner_text();
                        sss::scoped_ptr<sss::xml2::node> tmp_node(my_doc->create_cdata(value));
                        if (tmp_node.get()) {
                            p_node->clear();
                            p_node->append_child(tmp_node.release());
                            this->mark_modified();
            ...
            // 最好，给 sss::xml2::node 再添加一个parser方法，让能够想
            ...
            inline std::string set_inner(const std::string& path, const std::string& xml_str)
            {
                try {
                    ...
                    if (p_node) {
                        ret = p_node->inner_text();
                        sss::scoped_ptr<sss::xml2::node> sub(my_doc->create_subtree(xml_str));
                        if (sub.get()) {
                            p_node->clear();
                            p_node->append_child(sub.release());
                        }
            ...
            inline std::string append_inner(const std::string& path, const std::string& xml_str)
            {
                try {
                    sss::xml2::node * p_node = my_doc->verify_path(path);
                    if (p_node) {
