...
#define TERMINAL_ENCODING "utf-8"
        ...
        node * xml_doc::append_child(node* child) // {{{1
        {
        ...
        void xml_doc::accept(sss::xml3::xml_visitor& v) // {{{1
        {
            v.visit(this);
        }
        ...
        void xml_doc::write(const std::string& fname, const char * sep) // {{{1
        {
            ...
            if (!ofs.good()) {
                ...
                oss << "sss::xml3::xml_doc::" << __func__ << ": file `" << fname << "` open failed to write!";
        ...
        node_text * xml_doc::create_text(const std::string& text, bool is_raw_data) // {{{1
        {
            if (!is_raw_data) {
                ...
                SSS_LOG_EXPRESSION(sss::log::log_DEBUG, text);
                return new node_text(text, this);
            }
        }
        ...
        node *      xml_doc::create_doctype(const std::string& data) // {{{1
        {
            ...
            // 这里写得真恶心！
            // 为什么？
            // 因为我在 xml_parser 解析doctype的时候，采取了完全保留的方式，
            // 从 <!DOCTYPE 到 > ——因为 doctype 可能会很复杂。
            //
            // 但这种预设，却可能给某些情况，带来麻烦；
            // 而且还和，comment、包括PI等设定不一致；
            //
            // 各类型的输出，见：
            // /home/sarrow/extra/sss/include/sss/xml3/xml_vprinter.cpp|84
            //
            // 总之，会造成麻烦……
            // <!DOCTYPE wallpapers SYSTEM "gnome-wp-list.dtd">
            std::ostringstream oss;
            std::string::const_iterator it_beg = data.begin();
            std::string::const_iterator it_end = data.end();
            if (sss::is_begin_with(it_beg, it_end, "<!")) {
                it_beg += 2;
            }
            if (sss::is_begin_with(it_beg, it_end, "DOCTYPE")) {
                it_beg += 7;
            }
            if (sss::is_end_with(it_beg, it_end, ">")) {
                it_end --;
            }
            sss::util::StringSlice<std::string::const_iterator> inner(it_beg, it_end);
            inner.trim();
            oss << "<!DOCTYPE " << inner << ">";
            return new node_doctype(oss.str(), this);
        }
